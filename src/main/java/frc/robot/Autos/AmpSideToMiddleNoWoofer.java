...
public class AmpSideToMiddleNoWoofer extends SequentialCommandGroup {
  ...
  public AmpSideToMiddleNoWoofer(SwerveSubsystem S_Swerve, Intake S_Intake, Arm S_Arm, Shooter S_Shooter, Climber S_Climber, Command[] Paths,  CommandXboxController notRealController, boolean isRedAlliance) {
    // Add your commands in the addCommands() call, e.g.
    // addCommands(new FooCommand(), new BarCommand());
    Pose2d initialPose; 
    if (isRedAlliance) {
        initialPose = PathPlannerPath.fromPathFile("AmpSideToMiddleNoWoofer1").flipPath().getPreviewStartingHolonomicPose();
      }
      else {
        initialPose = PathPlannerPath.fromPathFile("AmpSideToMiddleNoWoofer1").getPreviewStartingHolonomicPose();
      }
    addCommands(
      ...
      new ParallelRaceGroup(
        S_Swerve.findPathToPose(4.6, 6.35, 9, defaultContraints, isRedAlliance),
