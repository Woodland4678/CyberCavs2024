...
public class AmpSideToMiddleWoofer extends SequentialCommandGroup {
  ...
  public AmpSideToMiddleWoofer(SwerveSubsystem S_Swerve, Intake S_Intake, Arm S_Arm, Shooter S_Shooter, Climber S_Climber, Command[] Paths,  CommandXboxController notRealController, boolean isRedAlliance) {
    // Add your commands in the addCommands() call, e.g.
    // addCommands(new FooCommand(), new BarCommand());
    Pose2d initialPose; 
    if (isRedAlliance) {
        initialPose = PathPlannerPath.fromPathFile("AmpSideToMiddleWoofer1").flipPath().getPreviewStartingHolonomicPose();
      }
      else {
        initialPose = PathPlannerPath.fromPathFile("AmpSideToMiddleWoofer1").getPreviewStartingHolonomicPose();
      }
    addCommands(
      ...
      new InstantCommand(() -> S_Swerve.setHeadlights(true)),
      Paths[1],
      //S_Swerve.findPath(PathPlannerPath.fromPathFile("AmpSideToMiddleWoofer2"), new PathConstraints(4, 5, Units.degreesToRadians(540), Units.degreesToRadians(720)), 0.5),
      new AutoGrabNote(S_Swerve, S_Intake, true, notRealController, true),
      new ParallelRaceGroup(
      ...
      //S_Swerve.findPath(PathPlannerPath.fromPathFile("AmpSideToMiddleWoofer3"), new PathConstraints(4, 5, Units.degreesToRadians(540), Units.degreesToRadians(720)), 0.5),
      new AutoAim(S_Swerve, S_Shooter, S_Intake, notRealController, true),
      new InstantCommand(() -> S_Swerve.setHeadlights(true)),
      Paths[2],
      new AutoGrabNote(S_Swerve, S_Intake, true, notRealController, true),
      ...
      //S_Swerve.findPath(PathPlannerPath.fromPathFile("AmpSideToMiddleWoofer5"), new PathConstraints(4, 5, Units.degreesToRadians(540), Units.degreesToRadians(720)), 0.5),
      new AutoAim(S_Swerve, S_Shooter, S_Intake, notRealController, true)
    );
  }
}