...
public class ClearSideStraightToMiddle extends SequentialCommandGroup {
  ...
  public ClearSideStraightToMiddle(SwerveSubsystem S_Swerve, Intake S_Intake, Arm S_Arm, Shooter S_Shooter, Climber S_Climber, Command[] Paths,  CommandXboxController notRealController, boolean isRedAlliance) {
    // Add your commands in the addCommands() call, e.g.
    // addCommands(new FooCommand(), new BarCommand());
    Pose2d initialPose; 
    if (isRedAlliance) {
        initialPose = PathPlannerPath.fromPathFile("ClearSideStraightToMiddle1").flipPath().getPreviewStartingHolonomicPose();
      }
      else {
        initialPose = PathPlannerPath.fromPathFile("ClearSideStraightToMiddle1").getPreviewStartingHolonomicPose();
      }
    addCommands(
